      ...
      <div class="w-full max-w-2xl p-6 transition-transform duration-300 ease-in-out transform scale-75 bg-white rounded-lg shadow-lg">
        ...
        <div class="mt-4">
          ...
          <div>
            ...
            <input type="text" disabled v-model="clientSearch" placeholder="Rechercher un client" class="w-full p-2 mb-2 border border-gray-300 rounded-md focus:ring focus:ring-blue-500 focus:border-blue-500" />
            <!-- Sélecteur de clients -->
            <select id="client" disabled v-model="selectedClientId" @change="updateSelectedClient" class="w-full p-3 transition duration-150 ease-in-out border border-gray-300 rounded-md focus:ring focus:ring-blue-500 focus:border-blue-500">
        ...
        <div class="mt-4">
          ...
          <textarea id="comment" v-model="comment" disabled class="w-full p-3 transition duration-150 ease-in-out border border-gray-300 rounded-md focus:ring focus:ring-blue-500 focus:border-blue-500" placeholder="Entrez un commentaire"></textarea>
        </div>
        <div class="flex justify-between mt-4">
          <div class="w-full pr-2">
            <label for="start-date" class="block text-sm font-medium text-gray-700">Date de début:</label>
            <input id="start-date" type="date" disabled v-model="startDate" class="w-full p-3 transition duration-150 ease-in-out border border-gray-300 rounded-md focus:ring focus:ring-blue-500 focus:border-blue-500" />
          </div>
          <div class="w-full pl-2">
            <label for="end-date" class="block text-sm font-medium text-gray-700">Date de fin:</label>
            <input id="end-date" type="date" disabled v-model="endDate" class="w-full p-3 transition duration-150 ease-in-out border border-gray-300 rounded-md focus:ring focus:ring-blue-500 focus:border-blue-500" />
          </div>
        </div>
...
export default {
  ...
  mounted() {
    this.user_id = localStorage.getItem('userId'); // Récupérer l'ID de l'utilisateur connecté
    this.fetchResourcesAndEvents();
    this.fetchClients();
    this.fetchJoursFeries();
    this.fetchAgences();
  ...
  methods: {
    ...
    async fetchResourcesAndEvents() {
      try {
        const response = await axios.get(`${API_URL}/mycalendar?user_id=${this.user_id}`, {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        ...
        // Assurez-vous que `data.users` est un tableau
        if (Array.isArray(data.users)) {
          this.resources = data.users.map((user) => ({
            id: user.id,
            name: user.name,
            agence: user.agence,
            type: user.type, // Assurez-vous que le type est inclus
            interventions: user.interventions,
          }));

          // Extraire les événements à partir de `data.users.interventions`
          this.events = data.users.flatMap((user) =>
